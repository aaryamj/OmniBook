...
import ClientManagementHub from './components/ClientManagementHub';
...
export default function ClientManagement() {
    ...
    const handleExport = () => {
        ...
        setTimeout(() => {
            const csvContent = "Metric,Value\nReport,Client Directory & Governance\nFilter," + timeFilter + "\nGenerated At," + new Date().toISOString();
            
            const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
            const url = URL.createObjectURL(blob);
            const link = document.createElement("a");
            link.setAttribute("href", url);
            link.setAttribute("download", "OmniBook_Client_Governance_Report.csv");
                ...
                <main className="ml-sidebar-width pt-24 pb-gutter px-gutter min-h-screen">
                    <div className="max-w-container-max mx-auto space-y-gutter">
                        ...
                        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 relative">
                            <div>
                                <h2 className="text-2xl sm:text-headline-lg font-headline-lg font-bold text-primary">Client Management</h2>
                                <p className="font-body-md text-body-md text-on-surface-variant">Real-time surveillance, personal records, and governance across all registered clients.</p>
                            </div>
                            <div className="flex flex-wrap sm:flex-nowrap gap-3 w-full sm:w-auto">
                                ...
                                <button 
                                    ...
                                    className={`px-4 py-2 font-label-md text-label-md rounded flex items-center gap-2 transition-all shadow-lg min-w-[170px] justify-center ${
                        ...
                        {/* Client SaaS KPIs (Omitted MRR; Added Total Bookings) */}
                        <ClientOverviewCards timeFilter={timeFilter} />
                        
                        {/* Client Management Hub Table */}
                        <ClientManagementHub timeFilter={timeFilter} />
