...
export default function ProviderIdentitySetup() {
    ...
    useEffect(() => {
        ...
        // Fetch fresh provider profile from backend
        axios.get('http://localhost:8080/api/v1/provider/profile', {
            headers: { Authorization: `Bearer ${token}` }
        }).then(res => {
            if (res.data && res.data.success) {
                if (res.data.organizationType) {
                    localStorage.setItem('organizationType', res.data.organizationType);
                    setAndBroadcastOrgType(res.data.organizationType);
                }
                ...
                if (res.data.primaryAccentColor) {
                    localStorage.setItem('primaryAccentColor', res.data.primaryAccentColor);
                    applyTheme(res.data.primaryAccentColor);
                }

                const pName = res.data.fullName || localStorage.getItem('providerFullName') || '';
                if (pName) {
                    ...
                    setFormData(prev => ({
                        ...
                        credentials: res.data.credentials || prev.credentials,
                        medicalLicense: res.data.medicalLicense || prev.medicalLicense,
                        primarySpecialty: res.data.primarySpecialty || prev.primarySpecialty
                    }));
                }
                if (res.data.profilePictureUrl) {
                    setHeadshotPreview(res.data.profilePictureUrl);
                }
                if (res.data.licenseImageUrl) {
                    setLicensePreview(res.data.licenseImageUrl);
                }
            }
        }).catch(() => {
            // Fallback: strictly prioritize providerFullName over generic fullName
            const savedName = localStorage.getItem('providerFullName') || localStorage.getItem('fullName');
            const adminName = localStorage.getItem('adminFullName');
            // Ensure we don't accidentally load admin's name
            if (savedName && savedName !== adminName) {
    ...
    const handleSaveAndProceed = async () => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                if (combinedName) {
                    localStorage.setItem('fullName', combinedName);
                    localStorage.setItem('providerFullName', combinedName);
                }
                if (response.data.profile?.profilePictureUrl) {
                    localStorage.setItem('profilePicture', response.data.profile.profilePictureUrl);
                }
            ...
            <div className="flex flex-col lg:flex-row min-h-screen">
                    ...
                    <div className="max-w-md mx-auto w-full flex flex-col space-y-8">
                        ...
                        <div className="flex items-center space-x-6 py-2">
                            ...
                            <div className="flex items-center space-x-2">
                                ...
                                <span className="font-sans text-sm font-semibold font-bold text-[#0F172A]">{terms.providerSingular} Profiling</span>
                            </div>
                            <div className="h-px w-8 bg-outline-variant"></div>
                            <div className="flex items-center space-x-2 opacity-50">
                                ...
                                <span className="font-sans text-sm font-semibold text-on-surface-variant">{terms.serviceSingular} Setup</span>
                            </div>
                        </div>
                        {error && (
                        ...
                        <div className="space-y-10">
                            <div>
                                <h2 className="font-sans font-bold text-[#0F172A] mb-1 text-[24px]">{terms.providerSingular} Profile</h2>
                                <p className="text-[#45464d] font-sans text-[14px]">Verify your professional credentials and presence.</p>
                            </div>
                            ...
                            <div className="flex items-start gap-8">
                                ...
                                <div className="flex-1 py-2">
                                    ...
                                    <p className="text-[#45464d] text-[13px] leading-relaxed mb-4">
                                        Use a high-resolution, professional portrait. This image will be the primary visual for {terms.customerPlural.toLowerCase()} booking {terms.appointmentPlural.toLowerCase()}.
                                    </p>
                                    <button type="button" onClick={() => fileInputRef.current?.click()} className="flex items-center gap-2 text-[#0D9488] font-sans border border-[#0D9488]/20 px-4 py-2 rounded-lg hover:bg-[#0D9488]/5 transition-colors text-[12px] font-semibold cursor-pointer">
                            ...
                            <div className="grid grid-cols-2 gap-6">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <input
                                        className="w-full bg-[#f7f9fb] border border-[#e0e3e5] rounded-lg p-3 text-[#191c1e] focus:ring-[#0D9488] focus:border-[#0D9488] text-[14px]"
                                        type="text"
                                        placeholder={terms.facilityLabel === 'College' ? 'e.g. PhD, M.Ed' : terms.facilityLabel === 'Saloon' ? 'e.g. Master Stylist, Colorist' : 'e.g. MD, FACC'}
                                        value={formData.credentials}
                                        onChange={e => setFormData({ ...formData, credentials: e.target.value })}
                                    />
                                </div>
                                <div className="space-y-2">
                                    <label className="font-sans text-[12px] font-semibold tracking-wider text-[#45464d] uppercase">{terms.registrationLabel}</label>
                                    <input
                                        ...
                                        type="text"
                                        placeholder={terms.facilityLabel === 'College' ? 'EMP-98231' : terms.facilityLabel === 'Saloon' ? 'COS-4421' : 'MED-88392'}
                            ...
                            <div className="p-6 bg-[#f2f4f6] rounded-xl border border-[#e0e3e5] space-y-4">
                                <div className="flex items-center gap-2 text-[#0F172A]">
                                    <span className="material-symbols-outlined" style={{ fontVariationSettings: "'FILL' 1" }}>{terms.providersNavIcon}</span>
                                    <h3 className="font-sans font-bold text-[18px]">{terms.facilityLabel} Assignment</h3>
                                </div>
                                <div className="space-y-2">
                            ...
                            <div className="p-6 bg-[#f2f4f6] rounded-xl border border-[#e0e3e5] space-y-4 mt-6">
                                <div className="flex items-center gap-2 text-[#0F172A]">
                                    ...
                                    <h3 className="font-sans font-bold text-[18px]">{terms.registrationLabel} Verification</h3>
                                </div>
                                <p className="text-[#45464d] text-[13px] leading-relaxed">
                                    Please upload a scanned copy or clear photo of your official {terms.registrationLabel.toLowerCase()}. This is required for our internal verification process and will not be displayed publicly.
                                </p>
                                <div className="flex items-center gap-6">
                                    ...
                                    <div className="flex-1">
                                        <button type="button" onClick={() => licenseInputRef.current?.click()} className="flex items-center gap-2 text-[#0F172A] font-sans border border-[#0F172A]/20 px-4 py-2 rounded-lg hover:bg-[#0F172A]/5 transition-colors text-[12px] font-semibold cursor-pointer">
                                            <span className="material-symbols-outlined text-[18px]">upload_file</span>
                                            {licenseImage ? 'CHANGE DOCUMENT' : `UPLOAD ${terms.registrationLabel.toUpperCase()}`}
                                        </button>
                        ...
                        <div className="mt-12">
                            <button
                                ...
                                disabled={isLoading}
                                className="w-full bg-[#0F172A] text-white px-8 py-4 rounded-xl font-sans text-[14px] font-semibold hover:bg-[#1E293B] transition-all flex items-center justify-center gap-3 active:scale-[0.98] disabled:opacity-70 shadow-lg shadow-[#0F172A]/20 cursor-pointer"
                            >
                                {isLoading ? 'SAVING...' : `SAVE PROFILE & PROCEED TO ${terms.serviceSingular.toUpperCase()} SETUP`}
                ...
                <div className="hidden lg:flex w-1/2 bg-surface flex-col items-center justify-center relative p-16">
                    ...
                    <div className="absolute top-10 flex items-center gap-3 px-5 py-2 bg-white/80 backdrop-blur-md rounded-full border border-white/50 shadow-sm">
                        ...
                        <span className="font-sans text-[#0D9488] tracking-[0.2em] text-[10px] font-black uppercase">LIVE {terms.customerSingular.toUpperCase()}-FACING PREVIEW</span>
                    </div>
                        ...
                        <div className="bg-white rounded-[2rem] p-8 shadow-2xl shadow-[#0F172A]/10 flex flex-col items-center text-center border border-[#c6c6cd]/10">
                            ...
                            <div className="flex items-center gap-2 mb-1 flex-wrap justify-center">
                                <h2 className="font-sans text-[24px] font-bold text-[#0F172A]">
                                    {terms.facilityLabel === 'Clinic' ? 'Dr. ' : ''}{formData.firstName || (terms.facilityLabel === 'Clinic' ? 'Sarah' : 'Alex')} {formData.lastName || (terms.facilityLabel === 'Clinic' ? 'Jenkins' : 'Taylor')}{formData.credentials ? `, ${formData.credentials}` : ''}
                                </h2>
                            ...
                            <button type="button" className="w-full bg-[#0D9488] text-white py-4 rounded-2xl font-bold text-[16px] hover:brightness-110 transition-all shadow-lg shadow-[#0D9488]/20 active:scale-[0.98] cursor-pointer">
                                BOOK {terms.appointmentSingular.toUpperCase()}
                            </button>
                            ...
                            <div className="mt-6 flex gap-4 w-full border-t border-[#eceef0] pt-6">
                                ...
                                <div className="flex-1">
                                    <p className="text-[10px] font-sans text-[#45464d] font-semibold uppercase mb-1">{terms.facilityLabel}</p>
                                    <p className="font-bold text-[#0F172A] text-[14px] truncate">{orgName || (terms.facilityLabel === 'College' ? 'Lincoln Academy' : terms.facilityLabel === 'Saloon' ? 'Elite Styles Studio' : 'North Heart Center')}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <footer className="absolute bottom-10 px-6 text-center">
                        <p className="text-[#45464d]/80 font-sans max-w-[320px] text-[13px]">
                            Changes to your professional profile are reflected instantly in the <span className="font-bold text-[#0F172A]">OmniBook</span> {terms.customerSingular.toLowerCase()} portal.
                        </p>
                    </footer>
                </div>
            </div>
