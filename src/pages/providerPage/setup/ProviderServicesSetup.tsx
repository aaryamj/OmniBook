...
export default function ProviderServicesSetup() {
    ...
    const getDefaultServiceName = () => {
        if (terms.facilityLabel === 'College') return 'Lecture / Office Hours';
        if (terms.facilityLabel === 'Saloon') return 'Haircut & Styling';
        return 'Initial Consultation';
    };

    const [services, setServices] = useState<ProviderService[]>([
        { serviceName: getDefaultServiceName(), durationMinutes: 30, fee: 1000, isTelemedicine: true }
    ]);
    ...
    useEffect(() => {
        ...
        axios.get('http://localhost:8080/api/v1/provider/profile', {
            ...
            console.error("Failed to sync provider profile in services setup:", err);
        });
    }, []);
    ...
    useEffect(() => {
        setServices(prev => {
            if (prev.length === 1) {
                const currentName = prev[0].serviceName;
                const defaultNames = ['Lecture / Office Hours', 'Haircut & Styling', 'Initial Consultation', 'Standard Follow-up', 'Consultation / Session'];
                if (defaultNames.includes(currentName) || !currentName) {
                    return [{ ...prev[0], serviceName: getDefaultServiceName() }];
                }
            }
            return prev;
        });
    ...
    const handleAddService = () => {
        const defaultAddName = terms.facilityLabel === 'College' ? 'Lab Session' : terms.facilityLabel === 'Saloon' ? 'Color & Treatment' : 'Standard Follow-up';
        setServices([
            ...services,
            { serviceName: defaultAddName, durationMinutes: 15, fee: 500, isTelemedicine: false }
        ]);
    ...
    const handleLaunchWorkspace = async () => {
        ...
        try {
            ...
            const response = await axios.post('http://localhost:8080/api/v1/provider/services', {
                services: services.map(s => ({
                    serviceName: s.serviceName,
                    durationMinutes: s.durationMinutes,
                    fee: s.fee,
                    isTelemedicine: s.isTelemedicine,
                    category: terms.serviceSingular,
                    isActive: true
                }))
            }, {
                headers: {
            ...
            setError(err.response?.data?.message || 'Failed to submit services');
        } finally {
            setIsLoading(false);
        }
    };
                    ...
                    <div className="max-w-md mx-auto w-full flex flex-col space-y-8">
                        ...
                        <div className="flex items-center space-x-6 py-2">
                            ...
                            <div className="flex items-center space-x-2">
                                ...
                                <span className="font-sans text-sm font-semibold font-bold text-[#0F172A]">{terms.serviceSingular} Setup</span>
                            </div>
                        </div>
                            ...
                            {isSubmitted ? (
                                <div className="flex flex-col items-center justify-center h-full space-y-6 text-center py-12">
                                    ...
                                    <p className="text-[#45464d] max-w-sm text-[16px]">
                                        Your {terms.providerSingular.toLowerCase()} application and {terms.serviceSingular.toLowerCase()} details are currently under review by the {terms.facilityLabel.toLowerCase()} administration team.
                                    </p>
                                    <div className="bg-[#f2f4f6] p-6 rounded-xl border border-[#c6c6cd]/30 mt-6 max-w-sm w-full shadow-sm text-left">
                                        ...
                                        <ul className="space-y-4">
                                            <li className="flex items-start gap-3">
                                                ...
                                                <p className="font-sans text-sm text-[#45464d]">The {terms.facilityLabel.toLowerCase()} admin verifies your credentials and {terms.serviceSingular.toLowerCase()}s.</p>
                                            </li>
                                            <li className="flex items-start gap-3">
                                            ...
                                            <li className="flex items-start gap-3">
                                                ...
                                                <p className="font-sans text-sm text-[#45464d]">You will receive an email to access your Live {terms.providerSingular} Dashboard.</p>
                                            </li>
                                        </ul>
                                    </div>
                                </div>
                                ...
                                <>
                                    {/* Titles */}
                                    <div className="mb-10">
                                        <h1 className="text-[30px] leading-[38px] tracking-[-0.02em] font-bold text-black mb-2">Define Your {terms.servicePlural || `${terms.serviceSingular}s`}</h1>
                                        <p className="text-[14px] text-[#45464d]">Create the bookable {terms.appointmentPlural.toLowerCase()} {terms.customerPlural.toLowerCase()} will see on your public profile.</p>
                                    </div>
                                    ...
                                    <form className="space-y-6">
                                        {services.map((service, index) => (
                                            ...
                                            >
                                                <div className="space-y-1.5">
                                                    <label className="text-[12px] font-semibold tracking-[0.05em] text-[#45464d] uppercase">{terms.serviceSingular} Name</label>
                                                    <input
                                                ...
                                                <div className="grid grid-cols-2 gap-4">
                                                    ...
                                                    <div className="space-y-1.5">
                                                        <label className="text-[12px] font-semibold tracking-[0.05em] text-[#45464d] uppercase">{terms.serviceSingular} Fee (Rs.)</label>
                                                ...
                                                <div className="flex items-center justify-between p-4 bg-[#f2f4f6] border border-[#c6c6cd] rounded-lg">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <span className="text-[14px] font-medium text-black">
                                                            {terms.facilityLabel === 'Clinic' 
                                                                ? 'Offer via Telemedicine / Video Call' 
                                                                : terms.facilityLabel === 'College' 
                                                                    ? 'Offer via Online Lecture / Video' 
                                                                    : 'Offer via Virtual / Online Session'}
                                                        </span>
                                                    </div>
                                        ...
                                        <button
                                            ...
                                            className="flex items-center gap-2 text-[12px] font-bold text-[#40c2fd] hover:text-[#00668a] transition-all py-2 cursor-pointer"
                                        >
                                            <span className="material-symbols-outlined text-[18px]">add_circle</span>
                                            + Add Another {terms.serviceSingular}
                                        </button>
                                        ...
                                        <div className="pt-8 pb-4">
                                            <button
                                                ...
                                                disabled={isLoading}
                                                className="w-full bg-[#131b2e] text-white py-4 rounded-xl text-[20px] font-bold hover:bg-slate-800 transition-all shadow-lg shadow-[#131b2e]/20 disabled:opacity-70 active:scale-[0.98] cursor-pointer"
                                            >
                                                {isLoading ? 'LAUNCHING...' : `Launch ${terms.providerSingular} Workspace`}
                                            </button>
                                            <p className="mt-4 text-center text-[10px] font-semibold text-[#79849a] uppercase tracking-widest">
                                                By launching, you agree to the {terms.facilityLabel}'s Revenue Sharing terms.
                                            </p>
                                        </div>
                                    </form>
                                </>
                ...
                <div className="hidden lg:flex w-1/2 bg-surface flex-col items-center justify-center relative p-16">
                    ...
                    <div className="w-full max-w-sm flex flex-col items-center z-10">
                        <div className="flex items-center gap-2 mb-8 animate-pulse">
                            ...
                            <span className="text-[12px] font-black text-[#40c2fd] tracking-[0.2em] uppercase">{terms.customerSingular} Booking Preview</span>
                        </div>
                        ...
                        <div className="bg-white rounded-3xl p-8 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.1)] w-full border border-[#c6c6cd]/30 hover:-translate-y-1 transition-transform duration-300">
                            <div className="flex flex-col items-center mb-8">
                                ...
                                <h3 className="text-[20px] font-semibold text-black">Select a {terms.appointmentSingular} Type</h3>
                                <p className="text-[12px] font-semibold text-[#45464d] mt-1">Available for new {terms.customerPlural.toLowerCase()}</p>
                            </div>
                                    ...
                                    return isActive ? (
                                        ...
                                        <div key={index} className="relative group cursor-pointer">
                                            ...
                                            <div className="relative flex items-center justify-between p-4 bg-white border-2 border-[#40c2fd] rounded-2xl shadow-sm">
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <div>
                                                        <h4 className="text-[16px] font-bold text-black">{service.serviceName || `Untitled ${terms.serviceSingular}`}</h4>
                                        ...
                                        <div key={index} onClick={() => setActiveServiceIndex(index)} className="flex items-center justify-between p-4 bg-[#f2f4f6] border border-[#c6c6cd] rounded-2xl opacity-60 hover:opacity-100 transition-all cursor-pointer">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    <h4 className="text-[16px] font-bold text-[#45464d]">{service.serviceName || `Untitled ${terms.serviceSingular}`}</h4>
                            ...
                            <div className="mt-8 flex justify-center">
                                <button type="button" className="w-full bg-[#00668a] text-white py-3 rounded-xl text-[14px] font-bold shadow-md shadow-[#00668a]/10 hover:brightness-110 transition-all active:scale-[0.98] cursor-pointer">
                        ...
                        <p className="mt-8 text-[14px] text-[#45464d] text-center max-w-xs leading-relaxed">
                            Payments will be routed automatically to the {terms.facilityLabel.toLowerCase()}'s <span className="font-bold text-black underline decoration-[#40c2fd]/40 underline-offset-4">master ledger</span>.
                        </p>
                    </div>
                </div>
            </div>
