...
const Platform: React.FC = () => {
  return (
    <div className="bg-surface text-on-background font-body-md selection:bg-primary-fixed-dim selection:text-on-primary-fixed">
      <Navbar />
      ...
      <main className="pt-20">
          ...
          <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center relative z-10 w-full">
            <div className="space-y-6 text-center lg:text-left">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start pt-4">
                <button className="bg-tertiary text-on-tertiary font-bold px-8 py-3 rounded-[12px] hover:brightness-110 active:scale-95 transition-all flex items-center justify-center gap-2 shadow-lg">
                  View Solutions
                  <span className="material-symbols-outlined text-[20px]">arrow_forward</span>
                </button>
                <button className="bg-[#e1e8ff] text-primary font-bold px-8 py-3 rounded-[12px] hover:brightness-95 transition-all">
                  Watch Demo
                </button>
              </div>
            </div>
              ...
              <div className="bg-white rounded-[20px] p-3 shadow-[0_12px_40px_rgba(26,86,219,0.12)] border border-black/5 w-full">
                <div className="relative aspect-square w-full bg-[#d0d6e0] rounded-[12px] overflow-hidden flex items-center justify-center">
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div className="w-28 h-28 bg-primary rounded-[20px] flex items-center justify-center shadow-lg z-20 animate-pulse">
                    ...
                    <div className="absolute w-10 h-10 bg-[#b8c2d1] text-primary rounded-[10px] flex items-center justify-center shadow-md top-8 left-1/2 -translate-x-1/2 z-10">
                      <span className="material-symbols-outlined text-[20px]">smart_toy</span>
                    </div>
                    <div className="absolute w-10 h-10 bg-[#b8c2d1] text-primary rounded-[10px] flex items-center justify-center shadow-md bottom-8 left-1/2 -translate-x-1/2 z-10">
                      <span className="material-symbols-outlined text-[20px]">monitoring</span>
                    </div>
                    <div className="absolute w-10 h-10 bg-[#b8c2d1] text-primary rounded-[10px] flex items-center justify-center shadow-md left-8 top-1/2 -translate-y-1/2 z-10">
                      <span className="material-symbols-outlined text-[20px]">payments</span>
                    </div>
                    <div className="absolute w-10 h-10 bg-[#b8c2d1] text-primary rounded-[10px] flex items-center justify-center shadow-md right-8 top-1/2 -translate-y-1/2 z-10">
        ...
        <section className="py-16 px-margin-mobile md:px-margin-desktop bg-white">
          <div className="max-w-6xl mx-auto">
            <div className="text-center mb-12">
              ...
              <p className="text-base text-secondary max-w-2xl mx-auto">Specific tools designed to solve unique challenges across your organization.</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {pillars.map((pillar, idx) => (
                <div key={idx} className={`bento-card ${pillar.cardBg} p-8 rounded-[24px] shadow-sm hover:shadow-[0_12px_40px_rgba(26,86,219,0.12)] transition-all duration-300 ${pillar.border} flex flex-col items-start`}>
                  ...
                  <div>
                    <h3 className="text-sm text-on-background mb-1 font-medium">{pillar.role}</h3>
                    <p className="text-lg font-bold text-primary">{pillar.title}</p>
                  </div>
                  <ul className="space-y-3 w-full mt-10">
                    {pillar.features.map((feature, fIdx) => (
                      <li key={fIdx} className={`flex items-center gap-4 px-4 py-3 ${feature.bg} rounded-[10px]`}>
                        ...
                        <span className="text-sm font-medium text-on-background">{feature.text}</span>
                      </li>
                    ))}
                  </ul>
                </div>
          ...
          <div className="max-w-6xl mx-auto">
            <div className="flex flex-col md:flex-row items-center justify-between gap-12">
              <div className="max-w-md w-full">
                <h2 className="text-[28px] font-bold text-primary mb-4">Seamlessly Integrated.</h2>
                ...
                {integrations.map((integration, idx) => (
                  <div key={idx} className="bg-white w-[110px] h-[64px] rounded-[12px] flex items-center justify-center shadow-[0_4px_10px_rgba(26,86,219,0.03)] hover:shadow-md transition-shadow">
                    {integration.type === 'text' ? (
                      <span className="font-semibold text-[#94a3b8] text-base">{integration.content}</span>
                    ) : (
                      <span className="material-symbols-outlined text-[#94a3b8] text-2xl">{integration.content}</span>
                    )}
                  </div>
                ))}
              </div>
        ...
        <section className="max-w-5xl mx-auto mb-16 mt-8 px-4 md:px-0">
          <div className="bg-primary rounded-[24px] p-10 md:p-12 relative overflow-hidden shadow-lg flex flex-col md:flex-row items-center justify-between gap-8 text-left">
